...
Spawning output of `r spp` was estimated within each sub-area model and is reported here for each area (Tables \ref{tab:south-ssbES} and \ref{tab:north-ssbES}) and the combined estimates for the California stock (Table \ref{tab:es-ca-status}). Uncertainty is estimated within each model and is reported for the model area results south and north of Point Conception. The spawning output, in terms of millions of eggs, south of Point Conception was estimated at `r round(south_model$derived_quants[south_model$derived_quants$Label %in% paste0("SSB_", endyr), "Value"], 2)` and an unfished spawning output of `r round(south_model$derived_quants[south_model$derived_quants$Label == "SSB_Virgin", "Value"], 2)`. The spawning output north of Point Conception was estimated at `r round(north_model$derived_quants[north_model$derived_quants$Label %in% paste0("SSB_", endyr), "Value"], 2)` and unfished spawning output of `r round(north_model$derived_quants[north_model$derived_quants$Label == "SSB_Virgin", "Value"], 2)`. Across California, the stock for `r spp`, has a combined spawning output of `r sb_final`, unfished spawning output of `r sb0`, and a relative spawning output of `r 100*depl_final` percent. The stock is estimated to be above the management target at the start of 2023 (Figures \ref{fig:es-sb-all} and \ref{fig:es-depl-all}).  
