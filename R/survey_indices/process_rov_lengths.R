...
rov_length <- read.csv(file.path(dir, "Coppers_StereoSized_LatLonDepth.csv"))
...
rov_length <- rov_length[rov_length$Designation != "SMCA/Outside",  ]
rov_length$designation <- "MPA"
#rov_length$designation[rov_length$DesignationType == "Reference"] <- "Reference"
rov_length$designation[rov_length$Designation == "Reference"] <- "Reference"
...
remove_flag <- which(rov_length$Precision > 10)
...
table(rov_length$year, rov_length$area)
...
ggsave(filename = file.path(dir, "plots", "rov_length_by_area_year_april_data.png"),
       width = 10, height = 10)

ggplot(lengths_sub, aes(x = length_cm, fill = designation)) +
  geom_density(alpha = 0.4, lwd = 0.8, adjust = 0.5)+ 
  facet_wrap(c('area', 'year')) +
  scale_fill_viridis_d()
ggsave(file = file.path(dir, "plots", "rov_filtered_length_comparison_april_data.png"),
       width = 7, height = 7)
...
ggsave(filename = file.path(dir, "plots", "rov_length_by_area_designation_april_data.png"),
       width = 10, height = 10)
...
protect_s <- 0.73; open_s <- 1 - protect_s
...
write.csv(north, file = file.path(dir, "forSS", "north_weighted_lengths_rov_april_data.csv"), row.names = FALSE)

n <- protect_s * samp_south_mpa + open_s * samp_south_ref
south <- cbind(lfs_south_mpa[, 1:5], n, prop_s)
colnames(south)[6] <- "InputN"
write.csv(south, file = file.path(dir, "forSS", "south_weighted_lengths_rov_april_data.csv"), row.names = FALSE)

plot_comps(
  data = north,
  dir = dir,
  add_save_name = "north-april"
)

plot_comps(
  data = south,
  dir = dir,
  add_save_name = "south-april"
)