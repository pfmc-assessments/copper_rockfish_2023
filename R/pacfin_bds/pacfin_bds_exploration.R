...
sample_by_year_cond <- aggregate(lengthcm~area+year+cond, data, length, drop = FALSE)
sample_by_year_cond[is.na(sample_by_year_cond)] = 0
# Need to figure out how to calculate the proportion of samples
# by cond in each area by year
total_by_year <- prop_sample_by_year %>%
	group_by(area, year) %>%
 	summarize(total_by_year = sum(lengthcm)) 
total_by_year <- aggregate(lengthcm ~ year + area, data, length, drop = FALSE)
total_by_year[is.na(total_by_year)] = 0

total_dead_area <-  aggregate(lengthcm ~ year + area, sample_by_year_cond[sample_by_year$cond == "dead",], sum, drop = FALSE)
prop <- data.frame(
