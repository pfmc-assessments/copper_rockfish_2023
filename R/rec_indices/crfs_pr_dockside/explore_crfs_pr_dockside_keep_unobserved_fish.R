...
#data location
PR_files <-file.path(here(),"data","rec_indices","crfs_pr_dockside","raw_data","PR1Annual_Stitched_Together")
...
###Now get the total fish caught and per bag
rawspeciesInfo <- speciesCodes %>%
  dplyr::select(SpeciesAbbrv, CommonName) %>%
  rename(Species = SpeciesAbbrv)

test <- left_join(PR_catch, rawspeciesInfo)

test <- test %>% 
  filter(grepl("rockfish", CommonName) | 
           grepl("bocaccio", CommonName) |
         grepl("rockfish", CommonName) |
         grepl("cabezon", CommonName) | 
           grepl("greenling", CommonName) |
           grepl("chilipepper", CommonName))

PR_catch_RCG <- test %>%
  mutate(tot_Kept = Kept + UnObs) %>%
  group_by(ID) %>%
  summarise(tot_kept = sum(tot_Kept))

PR_bag_data <- left_join(PR_effort, PR_catch_RCG)
PR_bag_data <- PR_bag_data %>%
  filter(tot_kept > 0) %>%
  mutate(cpue = tot_kept/AnglersTotal) %>%
  mutate(over_limit = ifelse(cpue>10, 1,0))
#-------------------------------------------------------------------------------
#NOT USING THE i FILES since we want to look at recent history
...
raw_data <- PR_data %>%
  ...
  mutate(kept = ifelse(is.na(keptObs),0, keptObs)) %>%
  mutate(keptUnObs = ifelse(is.na(keptUnObs),0, keptUnObs))  %>%
  mutate(dead = ifelse(is.na(dead),0, dead))  %>%
  mutate(alive = ifelse(is.na(alive),0, alive))  %>%
  mutate(tot_kept = keptObs + keptUnObs) %>%
  mutate(cpue = tot_kept/anglers) %>%
  mutate(tot_fish_enc = dead + alive + keptObs + keptUnObs) %>%
  filter(tot_fish_enc > 0)
...
# i_data <- inn er_join(i_data, ispeciesInfo)
...
# 
save(raw_data, PR_bag_data, file = file.path(getwd(), "bag_change_visuals", "pr_data_for_bag_visuals.RData"))
