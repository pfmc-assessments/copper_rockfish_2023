...
dir <- file.path(here(), "data", "rec_indices", "crfs_cpfv_onboard", modelArea, model)
setwd(dir)
...
#district 1
pos_data1 <- onboard %>% filter(number.fish > 0) %>%
...
ggplot(pos_data, aes(x = reef.dist, y = cpue, colour = region)) +
  geom_jitter(alpha = 0.5, show.legend = FALSE) + facet_wrap(~region) +
  xlab("Distance to rocky subs")
ggsave(file.path(dir, "reef_distance.png"), height = 7, width = 7)
#Cumulative distribution of distance from reef
ggplot(pos_data, aes(reef.dist)) + xlab("Distance to rocky substrate") +
